...
#from recommender.Crawler_Reuters_Test import download_news_page
from recommender.Crawler_Z5News_Test import download_news_page
from recommender.execute import search_news
...
def crawler_pages(request):
    #try:
    source_select = str(request.GET['source_select'])

    if source_select == 'z5news':
        download_news_page()
    elif source_select == 'z5news_brasil':
        #TODO G1
        download_news_page()

    return render_to_response("config.html",{'messages':['Atualizado com sucesso !']})
    #except:
    #    return render_to_response("config.html", {'messages': ['Erro ao processar !']})
...
class RecomView(TemplateView):
    template_name = 'recom.html'

    def get_context_data(self, **kwargs):
        context = super(RecomView, self).get_context_data(**kwargs)
        ...
        try:
            if (source_select and query_processor_select and search_model_select and query_text):
                ...
                if docs_rel:
                    ...
                    docs_rel.object_list = DocumentData.objects.filter(idd__in=ids, source=source_type)
                        # .extra(
                        # select={'manual': 'FIELD(idd,%s)' % ','.join(map(str, ids))},
                        # order_by=['manual'])
...
class SearchRecommView(RecomView):
    def get_context_data(self, **kwargs):

        context = super(SearchRecommView, self).get_context_data(**kwargs)
        r_source_select = self.request.GET['r_source_select'] if 'r_source_select' in self.request.GET.keys() and \
                                                             self.request.GET['r_source_select'] else None
        email = self.request.GET['email'] if 'email' in self.request.GET.keys() and \
                                                             self.request.GET['email'] else None
        doc_recomm_select = self.request.GET['doc_recomm'] if 'doc_recomm' in self.request.GET.keys() and \
                                                                        self.request.GET['doc_recomm'] else None
        class_model_select = self.request.GET['class_model'] if 'class_model' in self.request.GET.keys() and \
                                                                  self.request.GET['class_model'] else None
        topics_select = self.request.GET.getlist('topics[]') if 'topics[]' in self.request.GET.keys() and \
                                                                self.request.GET['topics[]'] else None

        btn_submit = self.request.GET['recomm'] if 'recomm' in self.request.GET.keys() and self.request.GET['recomm'] else None
        # qid = int(self.request.GET['qid']) if 'qid' in self.request.GET.keys() and self.request.GET['qid'] else None
        # drid = int(self.request.GET['drid']) if 'drid' in self.request.GET.keys() and self.request.GET['drid'] else None
        ...
        try:
            if (r_source_select and doc_recomm_select and class_model_select and topics_select):
                print(r_source_select)
                print(doc_recomm_select)
                print(class_model_select)
                print(topics_select)


                exe_name = r_source_select+'_model_'+class_model_select+'_'+doc_recomm_select
                ...
                docs_list,metrics = search_news(exe_name, topics_select, r_source_select.upper(),down=(btn_submit=='recomm_update' and page<2),metrics=metrics_search)
                context['metrics'] =  metrics
                if (btn_submit == 'recomm_update' and page < 2):
                    messages.add_message(self.request, INFO, _('Importação de Notícias concluída com sucesso !'))
                ...
                query_request = RecommRequest()
                if email:
                    query_request.email = email
                    messages.add_message(self.request, INFO, _('Perfil de Assinante salvo com sucesso !'))

                query_request.source = r_source_select
                query_request.topics = '%s' % topics_select
                query_request.class_model = class_model_select
                query_request.doc_recomm = doc_recomm_select
                query_request.time_request = diff_in_seconds
                query_request.page = page
                query_request.save()
        ...
        self.request.session['r_source_select'] = r_source_select
        self.request.session['doc_recomm_select'] = doc_recomm_select
        self.request.session['class_model_select'] = class_model_select
        self.request.session['topics_select'] = topics_select
        # self.request.session['free_search'] = 'on' if free_search else ''
        self.request.session['metrics_search'] = 'on' if metrics_search else ''
...
try:
    ...
    print("Database not exist! please execute 'python manage.py migrate'")