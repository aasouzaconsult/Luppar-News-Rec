...
class E2V_AVG(object):
    ...
    def transform(self, X):
        return np.array([
                            np.mean([self.w2v[word] for word in words if word in self.w2v] or [np.zeros(self.dimensao)],
                                    axis=0)
                            for words in X
                            ])
...
class E2V_IDF(object):
    ...
    def fit(self, X, y):
        tfidf = TfidfVectorizer(analyzer=analyzer_nothing)
        tfidf.fit(X)
        maximo_idf = max(
            tfidf.idf_)  # Uma palavra que nunca foi vista (rara) então o IDF padrão é o máximo de idfs conhecidos (exemplo: 9.2525763918954524)
        #,
        self.wIDF = defaultdict(functools.partial(analyzer_nothing,maximo_idf),[(word, tfidf.idf_[i]) for word, i in tfidf.vocabulary_.items()])
        return self
    ...
    def transform(self, X):
        return np.array([
                            np.mean([self.w2v[word] * self.wIDF[word] for word in words if word in self.w2v] or [
                                np.zeros(self.dimensao)], axis=0)
                            for words in X
                            ])
