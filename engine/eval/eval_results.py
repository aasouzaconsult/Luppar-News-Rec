...
class EvaluationResult(object):
    ...
    def print_summary_statistic(self):
        ...
        print("------------------------------------------------")
        print(" Run: %s" % self.runid)
        print(" Number of Topics: %d" % self.results['num_q'])
        print("------------------------------------------------")
        print("    Total number of documents over all topics   ")
        print("------------------------------------------------")
        print(" Retrieved:  %d" % self.results['num_ret'])
        print(" Relevant:   %d" % self.results['num_rel'])
        print(" Rel_ret:    %d" % self.results['num_rel_ret'])
        print(" Map:        %f" % self.results['map'])
        print(" Gm_Map:     %f" % self.results['gm_map'])
        print(" Rprec:      %f" % self.results['Rprec'])
        print(" Bpref:      %f" % self.results['bpref'])
        print(" Recip_rank: %f" % self.results['recip_rank'])
        print("------------------------------------------------")
